...
-- (2 rows)
SELECT city.name, country.name, city.population, language
FROM city
JOIN country ON city.countrycode = country.code
...
-- (1 row)
SELECT country.name, continent, language
FROM country
...
-- (5 row)
SELECT country.name, language, continent
FROM country
JOIN countrylanguage ON country.code = countrylanguage.countrycode
...
SELECT continent, MAX(city.population) AS max_pop
FROM country
